...
if ($dockerConfig -match '8000:80') {
    Write-Host "✅ docker-compose.yml: Using port 8000" -ForegroundColor Green
} else {
    Write-Host "❌ docker-compose.yml: Port 8000 not configured" -ForegroundColor Red
}
...
if (Test-Path "client\config-overrides.js") {
    ...
    if ($configOverrides -match 'disableEsLint') {
        Write-Host "✅ config-overrides.js: ESLint disabled" -ForegroundColor Green
    } else {
        Write-Host "❌ config-overrides.js: ESLint not disabled" -ForegroundColor Red
    }
} else {
...
if (Test-Path "client\package.json") {
    ...
    if ($packageJson -match 'ESLINT_NO_DEV_ERRORS=true') {
        Write-Host "✅ package.json: Build script configured" -ForegroundColor Green
    } else {
        Write-Host "❌ package.json: Build script not configured" -ForegroundColor Red
    }
} else {
...
if (Test-Path "client\nginx.conf") {
    ...
    if ($nginxConfig -match 'try_files /index.html =404' -and $nginxConfig -notmatch 'return 301 /vtria-erp/') {
        Write-Host "✅ nginx.conf: Root access configured" -ForegroundColor Green
    } else {
        Write-Host "❌ nginx.conf: Root access not configured" -ForegroundColor Red
    }
} else {
...
try {
    ...
    if ($response.Content -match "VTRIA" -or $response.Content -match "nginx") {
        ...
        if ($response.Content -match "Welcome to nginx!") {
            Write-Host "⚠️  Showing nginx welcome page - React build needs fixing" -ForegroundColor Yellow
        }
    } else {
        Write-Host "❌ Unexpected response from server" -ForegroundColor Red
    }
} catch {
    Write-Host "❌ Cannot connect to port 8000: $_" -ForegroundColor Red
}

Write-Host "`nDeployment verification completed!" -ForegroundColor Cyan
Write-Host "If all checks pass ✅, try accessing: http://localhost:8000" -ForegroundColor White
