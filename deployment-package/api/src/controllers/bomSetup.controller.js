...
class BomSetupController {
    
    async setupBomTables(req, res) {
        let connection;
        ...
        try {
            ...
            // Insert sample BOM headers
            await connection.execute(`
                INSERT IGNORE INTO bom_headers (id, bom_number, title, status, created_by) VALUES
                (1, 'BOM-CP-001', 'Control Panel for Rolling Mill', 'active', 1),
                (2, 'BOM-HVAC-001', 'HVAC System for Warehouse', 'active', 1),
                (3, 'BOM-FAN-001', 'Ceiling Fans for Assembly Hall', 'active', 1)
            `);
            
            // Insert sample BOM components
            await connection.execute(`
                INSERT IGNORE INTO bom_components 
            ...
            // Verify creation
            const [bomCount] = await connection.execute('SELECT COUNT(*) as count FROM bom_components');
            const [headerCount] = await connection.execute('SELECT COUNT(*) as count FROM bom_headers');
            
            res.json({
                success: true,
                message: 'BOM tables created successfully',
                data: {
                    bom_headers_created: headerCount[0].count,
                    bom_components_created: bomCount[0].count
                }
            });
            
        } catch (error) {
            logger.error('Error setting up BOM tables:', error);
            res.status(500).json({
                success: false,
                message: 'Failed to setup BOM tables',
                error: error.message
            });
        } finally {
...
module.exports = new BomSetupController();